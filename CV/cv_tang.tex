...
\tabbedblock{
    {2021-present} \> Postdoctoral Scholar - University of California, Berkeley \\[5pt]
...
\section{Manuscripts}
\pubgroup{
    \manWithNum{Transactional Panorama: A Conceptual Framework for User Perception in Analytical Visual Interfaces}
    	\> Dixin Tang, Alan Fekete, Indranil Gupta, Aditya G. Parameswaran \\
    	\> \textbf{Under VLDB Revision}\\[5mm]
    	
    \manWithNum{Efficient and Compact Spreadsheet Formula Graphs}
    	\> Dixin Tang, Fanchao Chen, Christopher De Leon, Tana Wattanawaroon, Jeaseok Yun, \\ 
    	\> Srinivasan Seshadri, Aditya G. Parameswaran \\
    	\> \textbf{Under ICDE Submission}\\[5mm]
...
\tabbedblock{
    Conference Program Committee: SIGMOD'22, SIGMOD'22 (Demo Track), SIGMOD'23, EDBT'24  \\[5pt]
    Conference Reviewer: IEEE VIS'21 \\[5pt]
    Conference Session Chair: SIGMOD'22 \\[5pt]
...
\projectWithNum{FormS: A Python Library for Scalable Spreadsheet Formula Execution}{Mar. 2022-Present}
...
\projectWithNum{Transactional Panorama: Enhancing User Perception in Analytical Interfaces}{Sep. 2021-Present}
...
\projectWithNum{Taco: Efficient and Compact Spreadsheet Formula Graphs}{Jan. 2021-Present}
...
\projectWithNum{Modin: A Scalable Dataframe System}{Jan. 2021-Present}
...
\projectWithNum{Lux: Always-on Visualization Recommendations}{Jan. 2021-Present}
...
\projectWithNum{\db: Resource-efficient Database Execution}{Nov. 2017-Mar. 2021}
{Scalable data systems, while performant, 
consume many computing and memory resources, 
which subsequently introduces a high monetary cost 
if run on the cloud. 
I built a resource-efficient database, \db, 
to reduce resource usage while meeting a 
performance goal. 
...
which is defined as the time between when the data 
is ready (e.g. the last tuple arrives for one scheduled query)
...
and within the slackness, intelligently deciding 
which part of a scheduled query to execute lazily, 
which queries to share, and 
which intermediate states (e.g., a hash table for a hash join) 
to maintain. 
...
there will be (e.g., output tuples in 
earlier executions may be removed by later executions and 
will not contribute to the query result). 
Interestingly, the eager execution of different parts of a query 
will increase different amounts of query work for 
the same amount of reduced latency, 
which is quantified by a novel metric, \incability. 
In \inqp, we execute the sub-query that has lower \incability 
more lazily to reduce the total query work 
while meeting the performance goal. 

Next, I developed iShare, 
which selectively shares queries 
that query the same data and have different performance goals. 
...
requires the shared plan to meet the highest goal (i.e.
lowest latency constraint) and forces some participating queries 
to run more eagerly. Since eager incremental execution 
increases query work, sharing queries may offset the benefits 
of shared execution. iShare considers the two factors
together and finds efficient query plans that reduce 
redundant work across concurrent queries and also avoid the cost of 
eager incremental execution. 
%The experiments show that this approach 
%significantly reduces total work compared to the
%state-of-the-art multi-query optimizer. 
...
\projectWithNum{Adaptive Concurrency Control for Main-memory Databases}{Sep. 2015-Nov. 2017}
...
\projectWithNum{Structured Data Shuffling for Big Data Analytical Stacks}{Nov. 2013-Jan. 2015}
...
\projectWithNum{A Fast and Space-efficient Join Method for Log Processing in MapReduce}{Sep. 2012-Nov. 2013}
...
{\bf Affiliation} \> University of Chicago \\ % Referee company
{\bf Position} \> Professor, Liew Family Chairman of Computer Science \\ % Referee job title 
...
{\bf Affiliation} \> Institute of Computing Technology \\ % Referee company
{\bf Position} \> Associate Professor \\ % Referee job title 
